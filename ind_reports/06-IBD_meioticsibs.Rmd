...
```{r}
# reproject for aesthetics in figure but keep simpler planar (ESP4326) for geodist calcs
urban_reproj <- raster::projectRaster(urban, "+proj=utm +zone=34 +datum=WGS84 +units=m")
urban.cities <- ggplot() +
  ggspatial::layer_spatial(data = urban_reproj, aes(fill = stat(band1))) +
  geom_sf(data = DRCprov, color = "#737373", fill = NA, size = 0.05) +
  prettybasemap_nodrc_nonorth_dark +
...
# reproject for aesthetics in figure but keep simpler planar (ESP4326) for geodist calcs
parasiterate_reproj <- raster::projectRaster(parasiterate, "+proj=utm +zone=34 +datum=WGS84 +units=m")
...
ibD.meiotic_plot_prev <- ibD.meiotic.network %>% 
  ...
  tibble::as_tibble(.) %>% 
  ggplot() +
  ggspatial::layer_spatial(data = parasiterate_reproj, aes(fill = stat(band1))) +
