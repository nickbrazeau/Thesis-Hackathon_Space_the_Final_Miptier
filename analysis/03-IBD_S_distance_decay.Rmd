...
<!-- TODO This should all be one call or import -->
...
IBSdecay.df <- ibSdist %>%
  ...
  tidyr::gather(., key = "distcat", value = "distmetric", 6:8) %>%
...
ibSdist.long <- expand_distance_matrix(ibSdist) %>% 
  dplyr::left_join(., y = admhv001, by = "hv001.x")
...
IBSdecay.df <- ibSdist.long %>%
  dplyr::select(-c(dplyr::ends_with("_cat"))) %>%
  tidyr::gather(., key = "distcat", value = "distmetric", 6:8) %>%
...
## IBD vs. Distance Model Comparison
```{r}
#..............................................................
# Make Model Fit based on Log and linear RSS estimates
# going to liftover categories to somewhat representative numerics
#..............................................................
catlvls <- levels(factor(ibDdist$gcdistance_cat))
catnumlvls <- log( c(50, seq(100, 2000, by = 100), 3000) )

liftover <- tibble::tibble(
  distmetcat = catlvls,
  distmetcat_num = catnumlvls
)

ibDdist.logfit <- IBDdecay.df %>% 
  dplyr::left_join(., y = liftover)

ibDdist.logfit <- ibDdist.logfit %>% 
  dplyr::group_by(distcat) %>% 
  tidyr::nest()


ibDdist.logfit$modfit <- purrr::map(ibDdist.logfit$data, function(x){
  ret <- lm(meanIBD ~ distmetcat_num, 
            data = x)
  return(ret)
...
ibDdist.logfit$preds <- purrr::map(ibDdist.logfit$modfit, function(x){
  ret <- predict(x, interval = "predict")
  return(tibble::as_tibble(ret))
})


# get residual sum of squares
ibDdist.logfit$rss <- purrr::map(ibDdist.logfit$modfit, function(x){
  ret <- anova(x)["Residuals", "Sum Sq"]
  return(ret)
})

ibDdist.logfit.plotdf <- ibDdist.logfit %>% 
  dplyr::select(-c("modfit")) %>% 
  tidyr::unnest(cols = c("data", "preds", "rss"))


ibDdist.logfit.plot <- ibDdist.logfit.plotdf %>% 
  dplyr::mutate(rss = round(rss, 6), size = 4,
                family = "Helvetica", fontface = "bold") %>% 
  ggplot() + 
  geom_ribbon(aes(x = distmetcat_num, ymin = lwr, ymax = upr), fill = "#d8daeb", alpha = 0.8) +
  geom_point(aes(x = distmetcat_num, y = fit), color = "#542788") + 
  geom_point(aes(x = distmetcat_num, y = meanIBD), color = "#dfc27d", alpha = 0.8) +
  geom_text(aes(x = 6.3, y = 0.05, label = paste("RSS:", rss))) +
  facet_wrap(distcat ~ .) + 
  plot_theme +
  xlab("Distance Metric Category (numeric liftover)") +
  ylab("Mean IBD Estimate") +  
  labs(caption = "Truth in gold; Predictions in purple; prediction interval in light purple")


```

```{r, results='asis'}
plot(ibDdist.logfit.plot)
```
...
# get all files
llfiles <- list.files("~/Documents/MountPoints/mountedMeshnick/Projects/Space_the_Final_Miptier/results/distance_likelihoods/_rslurm_distance_likelihoods_recursion/",
