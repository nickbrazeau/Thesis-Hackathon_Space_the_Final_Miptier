...
perms.urb <- perms.urb %>% 
  dplyr::mutate(
    urbanmeandiff = purrr::map(results, median)
  ) %>% 
  tidyr::unnest(cols = urbanmeandiff)
...
obsurb <- dplyr::left_join(obsurb, urban.pts, by = "hv001.y") %>% 
  ...
  dplyr::mutate(urbanmediandiff =  (urbanmedian.x - urbanmedian.y)^2)


obsurb.urbnmeandiff <- mean(obsurb$urbanmediandiff)
...
summary(perms.urb$urbanmeandiff)
obsurb.urbnmeandiff
# will fix at 0 and 13 to capture it

urb.perm.plotObj <- ggplot() +
  geom_density(data = perms.urb, aes(x = urbanmeandiff), fill = "#969696", color = "#969696") 

perms.urb.pval <- quantile(perms.urb$urbanmeandiff, probs = c(0.025, 0.975))
# extract ggplot data
...
urb.perm.plotObj <- urb.perm.plotObj + 
  ...
  xlab("Permuted Urbanicity Mean Difference") + ylab("Sim Proportion") +
  plot_theme
...
prev.edge.dens <- ibD.meiotic.network %>% 
  ...
  dplyr::mutate(node_edge_density = igraph::degree(.)) %>%
  tibble::as_tibble(.) %>% 
  dplyr::select(c("name", "hv001", "node_edge_density"))
