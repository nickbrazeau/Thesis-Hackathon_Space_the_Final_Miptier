...
meiotic.buff <- sf::st_buffer(ibdmeioticsmpls.coords.pts, 
                              dist = 20*1e3) # m to km, min is 25 km in data
...
#..............................................................
# Lift over if no neighbors
#..............................................................
#TODO is this fair
ibD.meiotic.neighborhoodIBD$neighborhoodIBD.x[is.nan(ibD.meiotic.neighborhoodIBD$neighborhoodIBD.x)] <- 0
ibD.meiotic.neighborhoodIBD$neighborhoodIBD.y[is.nan(ibD.meiotic.neighborhoodIBD$neighborhoodIBD.y)] <- 0
...
# urban binary
#..............................................................
urban.bi.rstr <- raster::raster("~/Documents/GitHub/Space_the_Final_Miptier/data/derived_data/urbanicity_raster/urbanicity_binary.grd")

urbanbi <- rep(NA, nrow(coord.pts)) 
for (i in 1:nrow(coord.pts)) {
  urbanbi[i] <-  raster::extract(x = urban.bi.rstr,
                                     y = sf::as_Spatial(coord.pts$geometry[i]),
                                     buffer = coord.pts$urban_rura_ext[i],
                                     fun = sum, na.rm = T
  )
...
all.urban.pts.bi <- coord.pts %>% 
  dplyr::mutate(longnum = sf::st_coordinates(geometry)[,1],
                latnum = sf::st_coordinates(geometry)[,2],
                urbanbi = as.numeric(urbanbi >= 1)) %>% 
  dplyr::select(c("hv001", "longnum", "latnum", "urbanbi")) 

#..............................................................
# Subset all 
#..............................................................
urban.pts.bi <- all.urban.pts.bi %>% 
  dplyr::filter(hv001 %in% c(ibD.meiotic.neighborhoodIBD$hv001.x, ibD.meiotic.neighborhoodIBD$hv001.y))


```

**How many urban clusters vs. rural cluster**
```{r}
knitr::kable(table(all.urban.pts.bi$urbanbi))
...
colnames(urban.pts.bi)[1] <- "hv001.x"
ibD.meiotic.neighborhoodIBD <- dplyr::left_join(ibD.meiotic.neighborhoodIBD,
                                                urban.pts.bi, 
                                                by = "hv001.x")

colnames(urban.pts.bi)[1] <- "hv001.y"
ibD.meiotic.neighborhoodIBD <- dplyr::left_join(ibD.meiotic.neighborhoodIBD,
                                                urban.pts.bi, 
                                                by = "hv001.y")
...
### Pairwise-Urbanicity Binary
```{r}


IBD_meiotic_direct_plot.urb.bi <- ggplot() +
  geom_sf(data = DRCprov, color = "#737373", fill = "#525252", size = 0.05) +
  prettybasemap_nodrc_dark +
  geom_curve(data = ibD.meiotic.neighborhoodIBD_plotdf,
             aes(x = long.start, y = lat.start, 
...
jpeg("~/Documents/GitHub/Space_the_Final_Miptier/results/figures/DRC_meiotic_sibs_directionality_urban_binary.jpg", width = 8, height = 8, units = "in", res = 500)
plot(IBD_meiotic_direct_plot.urb.bi)
graphics.off()
```
...
### Pairwise-Urbanicity Continuous
```{r}
...
tofromcovar <- ibD.meiotic.neighborhoodIBD_plotdf %>% 
  dplyr::mutate(prevfrom = ifelse(hv001.x == from, prevmean.x, prevmean.y),
                ...
                urban_cont_from = ifelse(hv001.x == from, urbanmedian.x, urbanmedian.y),
                urban_cont_to = ifelse(hv001.x == to, urbanmedian.x, urbanmedian.y),
                urban_bi_from = ifelse(hv001.x == from, urbanbi.x, urbanbi.y),
                urban_bi_to = ifelse(hv001.x == to, urbanbi.x, urbanbi.y)) %>% 
  dplyr::select(c("from", "to", "hv001.x", "hv001.y", "smpl1", "smpl2", "malecotf", "neighborIBDdiff", "prevfrom", "prevto", "urban_cont_from", "urban_cont_to", "urban_bi_from", "urban_bi_to")) 
...
urbandiffhist <- tofromcovar %>% 
  dplyr::mutate(urbandiff = urban_cont_from - urban_cont_to) %>% 
  ggplot() + 
...
fromclsturban <- tofromcovar %>% 
  dplyr::select(c("from", "urban_cont_from")) %>% 
  dplyr::group_by(from) %>% 
  dplyr::summarise(
    edgedens = n(),
    urbanicity = unique(urban_cont_from) # same cluster by cluster
  )
...
toclsturban <- tofromcovar %>% 
  dplyr::select(c("to", "urban_cont_to")) %>% 
  dplyr::group_by(to) %>% 
  dplyr::summarise(
    edgedens = n(),
    urbanicity = unique(urban_cont_to) # same cluster by cluster
  )
...
fromcovars <- tofromcovar %>% 
  dplyr::select(c("from", "prevfrom", "urban_cont_from")) %>% 
  dplyr::group_by(from) %>% 
  dplyr::summarise(
    edgedens = n(),
    urban = unique(urban_cont_from), 
...
tocovars <- tofromcovar %>% 
  dplyr::select(c("to", "prevto", "urban_cont_to")) %>% 
  dplyr::group_by(to) %>% 
  dplyr::summarise(
    edgedens = n(),
    urban = unique(urban_cont_to), 
    prev = unique(prevto)
  )
