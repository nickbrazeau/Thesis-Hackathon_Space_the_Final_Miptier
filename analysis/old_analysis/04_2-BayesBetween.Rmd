...
adm1name.pairs <- readRDS("~/Documents/GitHub/Space_the_Final_Miptier/data/derived_data/ibDdist_prov_between.RDS") %>% 
  ...
  dplyr::select(c("adm1name.x", "adm1name.y", "F_between_mean"))

adm1name.pairs <- expand_distance_matrix(adm1name.pairs)
```
...
```
```{r, results='asis'}
provnames <- data.frame(nms = DRCprov$adm1name, 
                        coords = sf::st_centroid(DRCprov$geometry)) %>% 
  dplyr::mutate(
...
provplot <- ggplot() +
  ...
  ggrepel::geom_text_repel(data = provnames, 
                           aes(x = long, y = lat, label = nms),
...
cowplot::plot_grid(adm1name.pairs.plotObj, provplot, nrow = 1, align = "h")
...
provmodels <- readRDS("~/Documents/MountPoints/mountedScratchLL/Projects/Space_the_Final_Miptier/results/carbayes_between_prov_models/_rslurm_betweenMods_DICs/params.RDS") %>% 
  dplyr::select(c("formula", "outcome", "distcat"))

# get all files
provformlist <- list.files("~/Documents/MountPoints/mountedScratchLL/Projects/Space_the_Final_Miptier/results/carbayes_between_prov_models/_rslurm_betweenMods_DICs/",
...
```{r}
provmodels <- readRDS("~/Documents/MountPoints/mountedScratchLL/Projects/Space_the_Final_Miptier/results/carbayes_between_prov_models/carbayes_long_chains_BETWEEN.RDS")

names(provmodels) <- c("gcdisance", "roaddistance", "riverdistance")
provmodels <- provmodels %>% 
  dplyr::bind_rows(., .id = "distcat") 
