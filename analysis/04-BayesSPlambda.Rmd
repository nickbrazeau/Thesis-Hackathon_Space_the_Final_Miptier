...
## Hierarchial Bayesian Spatial Models for Province Gen-Geo Dist Slope 
...
# MCMC Diagnostics
#-------------------------------------------------------------------------
mod.framework.sp$mcmc.modsum <- purrr::map(mod.framework.sp$MCMC, print)  # note, print is overloaded here
mod.framework.sp$summresults <- purrr::map(mod.framework.sp$mcmc.modsum, "summary.results")
mod.framework.sp$summresults <- purrr::map(mod.framework.sp$summresults, function(x){
  pars <- rownames(x)
...
mod.framework.sp$modfit <- purrr::map(mod.framework.sp$mcmc.modsum, "modelfit")
mod.framework.sp$DICs <- purrr::map(mod.framework.sp$modfit, "DIC")
...
mod.framework.sp %>% 
  dplyr::select(c("formula", "distcat", "DICs")) %>% 
  dplyr::filter(distcat == "gcdistance") %>% 
  dplyr::mutate(formula = deparse(formula)) %>% 
  DT::datatable(.,
                rownames = F,
                options = list(
                  searching = F,
