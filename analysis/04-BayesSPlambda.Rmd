...
provmodels <- readRDS("~/Documents/GitHub/Space_the_Final_Miptier/results/carbayes_sp_dics/params.RDS") %>% 
  dplyr::select(c("formula", "distcat"))
...
# now get results
provformlistdf$results <- purrr::map(provformlistdf$path, readRDS) 
provmodels$results <- unlist(provformlistdf$results, recursive = F)
...
provmodels %>% 
  dplyr::select(c("formula", "distcat", "min_nEff", "DICs")) %>% 
